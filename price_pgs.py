...
def index_pgs():
    ...
    for i in range(len(df)):
        ...
        c.execute("""INSERT INTO index_price(broader_id,date,open,high,low,close,volume) VALUES (%s,%s,%s,%s,%s,%s,%s)""", (broader_id, str(df['Index Date'].iloc[i]).split(
    ...
    for i in range(len(df)):
        register_adapter(np.int64, psycopg2._psycopg.AsIs)
        c.execute("""INSERT INTO index_price(sectorial_id,date,open,high,low,close,volume ) VALUES (%s,%s,%s,%s,%s,%s,%s)""", (df['stk_id'].iloc[i], str(df['Index Date'].iloc[i]).split(
            ' ')[0], df['Open Index Value'].iloc[i], df['High Index Value'].iloc[i], df['Low Index Value'].iloc[i], df['Closing Index Value'].iloc[i], df['Volume'].iloc[i]))
...
def stock_pgs():
    ...
    for i in range(len(final_df)):
        register_adapter(np.int64, psycopg2._psycopg.AsIs)
        c.execute("""INSERT INTO stock_price(stock_id,date,open,high,low,close,volume ) VALUES ( %s,%s,%s,%s,%s,%s,%s)""", (final_df['stk_id'].iloc[i], str(
            final_df['date'].iloc[i]).split(' ')[0], final_df['OPEN'].iloc[i], final_df['HIGH'].iloc[i], final_df['LOW'].iloc[i], final_df['CLOSE'].iloc[i], final_df['TOTTRDQTY'].iloc[i]))
    conn.commit()
